import { runGiftcardsComCron } from "../src/cron.ts";
...
async function main() {
  ...
  // DRY_RUN=1 prints matched/unmatched product names without writing to the DB.
  // Use it to tune GIFTCARDS_BRAND_OVERRIDES from real Rakuten product names.
  const dryRun = ["1", "true", "yes"].includes(
    (process.env.DRY_RUN ?? "").toLowerCase()
  );

  await runGiftcardsComCron(env, { dryRun });
}

main()
  .then(() => process.exit(0))
  .catch((err) => {
    console.error("Giftcards.com cron failed:", err);
    process.exit(1);
  });
